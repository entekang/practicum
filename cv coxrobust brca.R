...
cr <- coxr(Surv(pfs_time, pfs_out) ~BRCA_Overall2 + LengthofSurgery_hour+ADJcycles+Ascites+DSscore, data = brca1)
#cr$x gives design matrix
xb <- cr$x %*% cr$coefficients
rel_haz <- exp(xb)
...
repcrossval <- function(repeats, folds, weight){
  ...
  for (k in 1:repeats){
    ...
    for (i in 1:folds){
      ...
      fit <- coxr(Surv(pfs_time, pfs_out) ~BRCA_Overall2 + LengthofSurgery_hour+ADJcycles+Ascites+DSscore, 
